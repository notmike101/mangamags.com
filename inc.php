<?php
    ...
    $dbInfo             = Array();
    // Lol, I wouldn't keep real login info here!
    ...
    //if(file_exists("/resources/demo")) die('This is a demo website.');
    //DEFINED("IS_SEARCH") ? require_once("../resources/apc.php") : require_once("resources/apc.php");
    //DEFINED("IS_SEARCH") ? require_once("../".$forumPath."/global.php") : require_once($forumPath."/global.php");
    //DEFINED("IS_SEARCH") ? require_once("../resources/class.MyBBIntegrator.php") : require_once("resources/class.MyBBIntegrator.php");
    //$MyBBI = new MyBBIntegrator($mybb,$db,$cache,$plugins,$lang,$config);

    //$userInfo = $MyBBI->getUser();
    $userInfo['uid'] == 0;
    $isLoggedIn = ($userInfo['uid'] != 0);
    ...
    function getChapterComments($series,$chapter) {
        global $dbInfo;
        $oCache = new CacheAPC();

        $comments = Array();

        if($oCache->getData('mangasite_comments_'.$series.'_'.$chapter)) {
            $comments = $oCache->getData('mangasite_comments_'.$series.'_'.$chapter);
        } else {
            $db = @mysqli_connect($dbInfo['host'],$dbInfo['username'],$dbInfo['password'],$dbInfo['db']);
            $series = mysqli_real_escape_string($db,$series);
            $chapter = abs(intval($chapter));
            ...
            if(!mysqli_connect_errno()) {
                if($result = @mysqli_query($db,"SELECT * FROM mangachapcomments WHERE series='".$series."' AND chapter='".$chapter."'")) {
                    while($row = mysqli_fetch_array($result)) {
                        $row['children'] = Array();
                        $comments[$row['id']] = $row;
                    }
            ...
            $oCache->setData('mangasite_comments_'.$series.'_'.$chapter, $uploads);
        }

        return $comments;
    }

    function getLatestUploads($amount = 5) {
        global $dbInfo;
        $oCache = new CacheAPC();
        ...
        if($oCache->getData('mangasite_latestUps'.$amount)) {
            $uploads = $oCache->getData('mangasite_latestUps'.$amount);
        } else {
            $db = @mysqli_connect($dbInfo['host'],$dbInfo['username'],$dbInfo['password'],$dbInfo['db']);
            if(mysqli_connect_errno()) {
                return 0;
            ...
            $oCache->setData('mangasite_latestUps'.$amount, $uploads);
        }
    ...
    function getSeriesInfo($series) {
        ...
        if($oCache->getData('mangasite_'.$series.'_info')) {
            ...
            while($row = mysqli_fetch_array($result)) {
                ...
                $info['name']        = $row['name'];
                $info['author']      = ($row['author'] == '' ? 'Unknown' : $row['author']);
                $info['artist']      = ($row['artist'] == '' ? 'Unknown' : $row['artist']);
                $info['chapters']    = $row['chapters'];
                $info['year']        = ($row['year'] == '' ? 'Unknown' : $row['year']);
                $info['direction']   = ($row['direction'] == '' ? 'Unknown' : $row['direction']);
                $info['status']      = ($row['status'] == '' ? 'Unknown' : $row['status']);
                $info['genre']       = ($row['genre'] == '' ? 'None' : $row['genre']);
                $info['description'] = ($row['description'] == '' ? 'No description has been written yet.' : $row['description']);
                $info['alternate']   = ($row['alternate'] == '' ? 'None' : $row['alternate']);
    ...
    function getAllSeries($start,$all = false) {
        global $dbInfo,$page_limit;
        $oCache = new CacheAPC();
        ...
        if($all == false) {
            if($oCache->getData('mangasite_mangas_'.$start)) {
                ...
                $start = abs(intval($start) - 1) * $page_limit;
                ...
                while($row = mysqli_fetch_array($result)) {
                    ...
                    $mangas[$index]['author']      = ($row['author'] == '' ? 'Unknown' : $row['author']);
                    $mangas[$index]['artist']      = ($row['artist'] == '' ? 'Unknown' : $row['artist']);
                    $mangas[$index]['chapters']    = $row['chapters'];
                    $mangas[$index]['year']        = ($row['year'] == '' ? 'Unknown' : $row['year']);
                    $mangas[$index]['direction']   = ($row['direction'] == '' ? 'Unknown' : $row['direction']);
                    $mangas[$index]['status']      = ($row['status'] == '' ? 'Unknown' : $row['status']);
                    $mangas[$index]['genre']       = ($row['genre'] == '' ? 'None' : $row['genre']);
                    $mangas[$index]['rating']      = $row['rate'];
                    $mangas[$index]['description'] = ($row['description'] == '' ? 'No description has been written yet.' : $row['description']);
                    $mangas[$index]['alternate']   = ($row['alternate'] == '' ? 'None' : $row['alternate']);
