...
require_once $forumPath."/inc/functions_indicators.php";
...
$theme['imglangdir'] = $fullSiteURL.'/'.$forumPath.'/'.$theme['imglangdir'].'/english/';
...
if($mybb->input['action'] == "thread")
{
	...
	else // Linear display
	{
		...
		if($ismod)
		{
			...
			{
				require_once $forumPath."/inc/functions_rebuild.php";
	...
	if($forumpermissions['canpostreplys'] != 0 && $mybb->user['suspendposting'] != 1 && ($thread['closed'] != 1 || is_moderator($fid)) && $mybb->settings['quickreply'] != 0 && $mybb->user['showquickreply'] != '0' && $forum['open'] != 0)
	{
		...
		{
			require_once $forumPath.'/inc/class_captcha.php';
	...
	eval("\$showthread = \"".$templates->get("integrateShowThread")."\";");
}
...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="<?php echo($fullSiteURL); ?>/resources/css/showthread.css" />
